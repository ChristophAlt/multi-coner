...
# Configure conda env
read -rp "Enter environment name [multi_coner]: " env_name
env_name=${env_name:-multi_coner}
...
if [ "$cuda_version" == "none" ]; then
    ...
    conda install -y pytorch=$pytorch_version cudatoolkit=$cuda_version -c pytorch -c nvidia
fi
