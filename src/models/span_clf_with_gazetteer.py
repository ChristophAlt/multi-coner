...
class SpanClassificationWithGazetteerModel(PyTorchIEModel):
    ...
    def _start_end_and_span_length_span_index(
        self, batch_size: int, max_seq_length: int, seq_lengths: Optional[Iterable[int]] = None, wikipedia_entities = None, gazetteer_features = None,
        ...
        for batch_index, seq_length in enumerate(seq_lengths):
            wikipedia_lookup = {
                (start, end, span_length): label for start, end, span_length, label in wikipedia_entities[batch_index]
            }
            gazetteer_lookup = {
                (start, end, span_length): features for start, end, span_length, features in gazetteer_features[batch_index]
            }
            for span_length in range(1, self.max_span_length + 1):
                for start_index in range(seq_length + 1 - span_length):
                    ...
                    gaz_feat = gazetteer_lookup.get((start_index, end_index, span_length), all_zeros)
    ...
    def forward(self, input_: TransformerSpanClassificationModelBatchEncoding) -> TransformerSpanClassificationModelBatchOutput:  # type: ignore
        ...
        (
            ...
            batch_size=batch_size, max_seq_length=seq_length, seq_lengths=seq_lengths, wikipedia_entities=wikipedia_entities, gazetteer_features=gazetteer_features,
        )
        ...
        combined_embedding = torch.cat(
            (start_embedding, end_embedding, span_length_embedding, wiki_embedding, gazetteer_embedding), dim=-1
